...
Browser requests are allowed from any origin in v1. Gin returns wildcard CORS
headers for public responses and explicitly permits `GET`, `POST`, and the
`X-Api-Key` request header. API Gateway owns mock `OPTIONS` preflights and adds
the wildcard origin to default 4xx and 5xx gateway responses. The browser sends
...
The stack outputs `ManagementAPICommandQueueURL`,
`ManagementAPICommandQueueARN`, and `ManagementAPICommandDLQURL`. After
deployment, a user can submit commands:
...
domain, and `cmd/ts6-management-api` remains the manual composition root.
...
For local use, copy `.env.example` to `.env` and replace the placeholder AWS
profile and account ID. The service loads `.env` from its working directory
before reading configuration. Existing process variables take precedence, so
CI and Lambda environment configuration are never overwritten. The example
uses an AWS profile instead of storing access keys in the file.

```bash
cp .env.example .env
go run ./cmd/ts6-management-api
```

The `.env` file is ignored by Git and excluded from the Docker build context.
Malformed entries fail startup without logging their values.
...
The independent browser application lives in
[`../ts6-management-web`](../ts6-management-web/README.md). It calls this API
directly through API Gateway; it is not bundled into this Go module or Docker
image.
