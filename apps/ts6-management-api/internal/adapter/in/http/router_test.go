...
func TestNewAllowsBrowserRequestsWithAPIKey(t *testing.T) {
	t.Parallel()

	service := &fakeLifecycle{status: lifecycle.Status{State: lifecycle.StateStopped}}
	router := newRouter(t, service, slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	t.Run("preflight", func(t *testing.T) {
		...
		if got := recorder.Header().Get("Access-Control-Allow-Headers"); !strings.EqualFold(got, "X-Api-Key") {
			t.Errorf("Access-Control-Allow-Headers = %q, want X-Api-Key", got)
		}
	})
}
